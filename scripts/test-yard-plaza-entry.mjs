...
await test("actual AvatarEditSheet still opens the new Wardrobe and forwards a saved paperdoll", async () => {
  ...
  const oldSavedLook = Object.freeze({ ...DEFAULT_LOOK, face: "rabbit", top: "dress", bottom: "pants", bottomColor: "navy" });
  ...
  assert.equal(oldView.props.initial.version, 2, "The restored editor keeps the original v2 format");
  for (const field of ["face", "top", "bottom", "bottomColor"]) assert.equal(oldView.props.initial[field], oldSavedLook[field]);
  for (const field of ["bag", "glasses", "neckwear"]) assert.equal(Object.hasOwn(oldView.props.initial, field), false);
