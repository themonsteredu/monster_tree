...
export function VillageClient({ settings, buildings, studentName, totalPoints }: Props) {
  ...
  return (
    ...
    >
      {/* 상단 정보 */}
      <header className="absolute top-0 inset-x-0 z-20 px-4 pt-4 flex items-start justify-between gap-3">
        <h1
        ...
        <div className="bg-black/45 backdrop-blur-sm rounded-full px-3 py-1.5 text-xs font-semibold flex items-center gap-2">
          <span className="text-white/90">{studentName}</span>
          <span className="text-amber-300">⭐ {totalPoints}</span>
        </div>
      </header>

      {/* 마을 캔버스 — 모바일 화면 안에 한 번에 보이게 */}
      <div
        ...
        {visible.map((b) => {
          ...
          return (
            ...
            >
              <div className="w-full transition-transform group-hover:scale-105 group-active:scale-95">
                {b.image_url ? (
                  ...
                  <img
                    src={b.image_url}
                    alt={b.name}
                    className="w-full h-auto object-contain drop-shadow-[0_6px_12px_rgba(0,0,0,0.35)]"
                    onError={(e) => {
              ...
              <div className="mt-1 inline-block bg-black/55 text-white text-[11px] font-semibold px-2.5 py-1 rounded-full">
                {b.name}
      ...
      {toast && (
        <div
          role="status"
          aria-live="polite"
          className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 bg-black/80 text-white text-sm font-medium px-4 py-2.5 rounded-full shadow-lg animate-[fadein_0.15s_ease-out]"
        >
          {toast}
        </div>
      )}
