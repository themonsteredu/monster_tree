...
export function parsePaperDollLook(raw: unknown): PaperDollLook | null {
  ...
  if (value.kind !== "paperdoll" || value.version !== 2) return null;
  const fields = Object.keys(LOOK_OPTIONS) as LookField[];
  if (Object.keys(value).length !== fields.length + 2) return null;
  ...
  for (const field of fields) {
    ...
    if (!(LOOK_OPTIONS[field] as readonly unknown[]).includes(value[field])) return null;
    clean[field] = value[field];
  }
  return clean as PaperDollLook;
}

/** Legacy avatars get a new starter look only when entering the new wardrobe. */
